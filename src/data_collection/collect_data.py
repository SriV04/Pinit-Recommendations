...
HTTP_SESSION.headers.update({
    "User-Agent": "open-food-map-collector/1.0"
})
...
MAX_RETRIES         = 5
TRANSIENT_STATUS_DELAYS = {
    "UNKNOWN_ERROR": 2.0,
    "OVER_QUERY_LIMIT": 5.0,
    "RESOURCE_EXHAUSTED": 5.0,
    "INVALID_REQUEST": 2.0,  # often seen when page tokens are not ready yet
}
NON_RETRYABLE_STATUSES = {"REQUEST_DENIED", "PERMISSION_DENIED"}
...
def _sleep_with_jitter(base: float):
    """Sleep for `base` seconds plus a small random jitter to avoid sync bursts."""
    if base <= 0:
        return
    jitter = random.uniform(0.8, 1.2)
    time.sleep(base * jitter)

def safe_request(url: str, params: Dict, allowed_statuses: Tuple[str, ...] = ("OK",), max_retries: int = MAX_RETRIES) -> Dict:
    """
    Perform a Google Places API request with retries, handling both HTTP-level issues
    and Places-specific status codes. Returns the JSON payload if status is allowed.
    """
    last_error = ""
    for attempt in range(1, max_retries + 1):
        try:
            resp = HTTP_SESSION.get(url, params=params, timeout=30)
            resp.raise_for_status()
        except requests.RequestException as exc:
            last_error = f"HTTP error: {exc}"
            print(f"[warn] {last_error} (attempt {attempt}/{max_retries})")
            backoff = min(REQUEST_SLEEP_S * (2 ** (attempt - 1)), 15.0)
        ...
        if not status or status in allowed_statuses:
            return data

        if status in NON_RETRYABLE_STATUSES:
            err_msg = data.get("error_message") or "non-retryable error"
            raise RuntimeError(f"Google Places error {status}: {err_msg}")

        should_retry = False
        wait_hint = TRANSIENT_STATUS_DELAYS.get(status, REQUEST_SLEEP_S)
        if status in TRANSIENT_STATUS_DELAYS:
            should_retry = True
        if status == "INVALID_REQUEST" and "pagetoken" in params:
            should_retry = True
            wait_hint = max(wait_hint, PAGE_TOKEN_WAIT_S)

        if should_retry:
            err_msg = data.get("error_message") or ""
            print(f"[warn] Google status {status} (attempt {attempt}/{max_retries}). {err_msg} Retrying...")
            backoff = min(max(wait_hint, REQUEST_SLEEP_S * (2 ** (attempt - 1))), 20.0)
            _sleep_with_jitter(backoff)
            continue

        err_msg = data.get("error_message") or "Unknown API error"
        last_error = f"Google status {status}: {err_msg}"
        print(f"[warn] {last_error}")

    raise RuntimeError(f"Failed after retries. Last error: {last_error or 'unknown error'}")
...
def places_nearby_all_pages(lat: float, lon: float, radius_m: int, place_type: str) -> list[Dict]:
    ...
    data = safe_request(NEARBY_URL, params, allowed_statuses=("OK", "ZERO_RESULTS"))
    ...
    while True:
        ...
        data = safe_request(NEARBY_URL, params2, allowed_statuses=("OK", "ZERO_RESULTS"))
        results = data.get("results", []); collected.extend(results)
    return collected
...
def get_place_details(place_id: str) -> Optional[Dict]:
    ...
    data = safe_request(DETAILS_URL, params, allowed_statuses=("OK", "ZERO_RESULTS", "NOT_FOUND"))
...
def main():
    ...
    for i in range(start_idx, total_points):
        ...
        try:
            ...
            print(f"[info] Retrieved {len(results)} places from Nearby search.")
        ...
        for pid in new_ids:
            ...
            # Format opening hours text (join weekday descriptions)
            opening_hours_text = " | ".join(weekday_text) if weekday_text else ""
            
            # Format periods as JSON string
            opening_hours_periods = json.dumps(periods, ensure_ascii=False) if periods else ""
            ...
            details_row = {
                ...
                "open_now": str(open_now) if open_now is not None else "",
